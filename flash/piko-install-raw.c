...
#define START_ADDR   917504
#define CHUNK_SIZE   524288
#define MAX_KERNEL   1294336
...
int main(int argc, char *argv[])
{
    ...
    if (src < 0) {
        puts_("Piko Install: zImage not found on card\n");
        _exit_(1);
    }
    ...
    if (datasize > MAX_KERNEL) {
        puts_("Piko Install: kernel too big for flash slot: ");
        putnum(datasize);
        puts_("\n");
        _exit_(1);
    }

    puts_("Piko Install: flashing zImage (");
    putnum(datasize);
    ...
    if (src < 0) {
        puts_("Piko Install: verify: cannot reopen zImage\n");
        _exit_(1);
    }
    ...
    puts_("Piko Install: VERIFY OK. Kernel flash confirmed correct. Power off and reboot.\n");
    _exit_(0);
    return 0;
}
