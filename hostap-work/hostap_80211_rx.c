		     ...
		     struct hostap_80211_rx_status *rx_stats)
{
	...
	/* iwspy per-station signal monitoring (wireless_spy_update() and the
	 * net_device.wireless_data plumbing it needed) was removed from
	 * current mainline along with struct iw_public_data — it has no
	 * bearing on basic association/data flow, so it's dropped here
	 * rather than resurrecting that core netdevice.h field. */
