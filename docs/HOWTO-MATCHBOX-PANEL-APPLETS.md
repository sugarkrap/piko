...
| `mb-applet-pikaffeine` | Caffeine-style toggle: empty cup (off, the default at every session start) / full cup (on). On tells `brightd` to stop idle dimming/blanking/screensaving by creating `/tmp/brightd.inhibit` -- brightness hotkeys and the lid switch keep working regardless. Submodule `userspace/src/mb-applet-pikaffeine`, from `github.com/sugarkrap/mb-applet-pikaffeine`. Same plain-Makefile shape as `mb-applet-card`/`mb-brightness`; libmb is its only dependency. Built into `D_PIKAFFEINE`. See that repo's README. |
