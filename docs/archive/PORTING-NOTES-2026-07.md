# Porting notes — reviving Corgi/Husky against mainline (July 2026)

> **ARCHIVED — this was the project README until 2026-07-31.** Its "Status"
> section is frozen at 2026-07-19, when nothing had booted on hardware yet;
> the board has since booted to a full graphical desktop. Read `README.md`
> and `docs/BUILD-FROM-SCRATCH.md` for the current state, and treat the
> toolchain paths below (`/home/makaron/...`) as historical — the toolchain
> is now built in-repo by `tools/build-uclibc-toolchain.sh`.
>
> Kept because this is the record of **which mainline APIs broke and why**
> when the removed board files were pulled forward: the `.handle_irq` drop,
> the IrDA excision, the UDC platform_data → gpiod migration, the hostap
> `lib80211`/`michael_mic`/iwspy dependency chain, and the swnode migration
> for keypad/ads7846. That reasoning is still the best explanation of what
> the files under `modules/` actually change.
...
> PLEASE BEWARE THAT THIS IS INCOMPLETE.** See
> `docs/DEADLETTER-MTD2-MTD3.md` for a real account of a device this
> project actually bricked, and why.
