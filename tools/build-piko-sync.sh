...
# Builds piko-sync: piko-sync-server (cross-compiled, runs on the Zaurus)
# and piko-sync-client (built against the HOST's own FLTK, runs on the
# build machine). See userspace/src/piko-sync/README.md for what the app
# actually does.
#
...
# removes independently, and folding piko-sync into the shared payload
# later, once it has actually proven itself on hardware, is a small,
# separate, deliberate change to make then.
#
# WHY THE HOST TESTS RUN HERE: userspace/src/piko-sync/tests/ covers the
...
# PIKO_SYNC_SKIP_TESTS=1 to skip it if you are iterating and know what
# you are doing.
#
# Usage:
#   tools/build-piko-sync.sh [--server-only|--client-only] [--deploy user@host]
#
...
if [ ! -f "$SRC/protocol.h" ]; then
    echo "tools/build-piko-sync.sh: $SRC is missing protocol.h" >&2
    exit 1
fi
...
if [ "$BUILD_SERVER" = "1" ]; then
    if ! command -v "$TOOLCHAIN_BIN_DIR/$HOST-g++" >/dev/null 2>&1; then
        echo "tools/build-piko-sync.sh: no C++ cross compiler at $TOOLCHAIN_BIN_DIR/$HOST-g++" >&2
        exit 1
    fi
    if [ ! -f "$STAGE/usr/lib/libfltk.so.$FL_DSO_VERSION" ]; then
        echo "tools/build-piko-sync.sh: no staged libfltk at $STAGE" >&2
    ...
    case " $needed " in
        ...
        *)
            echo "tools/build-piko-sync.sh: piko-sync-server does not NEED libfltk.so.$FL_DSO_VERSION" >&2
...
if [ "$BUILD_CLIENT" = "1" ]; then
    if ! command -v fltk-config >/dev/null 2>&1; then
        echo "tools/build-piko-sync.sh: no host fltk-config -- skipping piko-sync-client" >&2
...
if [ -n "$DEPLOY_TARGET" ]; then
    if [ -z "$IPK_PATH" ]; then
        echo "tools/build-piko-sync.sh: --deploy needs the server to have been built" >&2
        exit 1
    fi
