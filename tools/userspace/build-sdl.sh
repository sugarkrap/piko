...
if [ ! -f "$SDL_TARBALL" ]; then
    ...
    curl -fL --http1.1 -o "$SDL_TARBALL.partial" "$SDL_URL"
...
echo ""
echo "    Deploy with tools/build-and-deploy.sh -- it ships this payload"
echo "    automatically once staged here, bootstrapping /lib/ld-uClibc*.so"
echo "    + /lib/libc.so if needed."
