...
void TransferTable::sync()
{
    if (!queue_)
        return;
    rows(queue_->count());
    /* Widths set here, not the constructor, so they follow a resize of
     * whatever tab/window contains this table -- same reasoning as
     * HistoryTable::set_rows in pikostore. */
...
Fl_Color TransferTable::status_color(TransferStatus s)
{
    ...
    case XFER_RECONNECTING:  return fl_rgb_color(0xE0, 0x90, 0x00); /* amber */
                               ...
                               int X, int Y, int W, int H)
{
    ...
    case CONTEXT_CELL: {
        ...
        if (C == COL_NAME) {
            ...
            fl_font(FL_HELVETICA, 12);
            /* An error's reason is more useful on this row than the word
             * "error" repeated -- there is no separate details column, so
             * this is the only place it can go. */
