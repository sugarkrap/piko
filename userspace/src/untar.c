/*
 * untar -- extract a plain (uncompressed) POSIX ustar archive to a
 * destination directory, entirely offline: no tar/gzip/unzip dependency
 * on this rootfs (same constraint piko-update.c documents). Unlike
 * piko-update.c this has no MANIFEST/MD5 ceremony, no zImage-full backup,
 * no lock, no reboot -- it's just "extract this archive somewhere",
 * for pushing plain data trees (e.g. xkeyboard-config) over SSH as one
 * transfer instead of hundreds of small per-file ones.
 *
 * Usage: untar <archive.tar> <destination-dir>
 *
 * Cross-compile (same toolchain as the rest of userspace/src):
 *   $GCC -march=armv5te -O2 -static -o untar untar.c
 *   arm-buildroot-linux-uclibcgnueabi-strip untar
 */
...
/* Rejects absolute paths and any ".." path component -- same guard as
 * piko-update.c, this archive shouldn't be able to write outside
 * destination root even if hand-edited or corrupt. */
...
int main(int argc, char **argv)
{
    ...
    for (;;) {
        ...
        if (typeflag == '2') { /* symlink */
            ...
            linkname[100] = '\0';
            unlink(dest_path); /* symlink() fails if dest_path already exists */
