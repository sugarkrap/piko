/*
 * Host-side tests for yaml_lite.h and manifest.h -- exercised against a
 * fixture directory standing in for the repo, and against the REAL
...
static void test_put_files_from_dir()
{
    ...
    write_fixture("pcm/sub/c.conf", "c"); /* must NOT appear: not recursive */
...
/* Populates just enough of the fixture repo for the ALWAYS-present entries
 * (no if_exists guard) to resolve -- kernel image, rcS, the panel action
 * scripts, etc. Deliberately leaves every conditional artifact (brightd,
 * mhz, the SSH/opkg/ALSA/SDL staged trees, dropbear) absent, so the tests
 * below can assert those are correctly left OUT of the plan by default. */
static void seed_minimal_repo(const DeployContext &ctx)
{
    ...
    write_fixture("repo/tools/kernel-modules.sh",
        "AUDIO_MODULES=\"\nsound/soundcore.ko\n\"\n"
    ...
    write_fixture("repo/tools/ssh-payload.sh",
...
static void test_conditional_files_absent_by_default()
{
    ...
    DeployContext ctx = make_fixture_context();
    seed_minimal_repo(ctx);
    /* brightd/mhz/flipd/pkillx/cardswap/kill deliberately NOT created. */
    ...
    /* opkg's whole group is gated on the staged opkg binary, never created
     * by seed_minimal_repo -- none of it should appear. */
    ...
    /* The always-present files, on the other hand, must be there. */
