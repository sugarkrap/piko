/*
 * Host-side tests for settings.h -- the persistent key=value store
 * behind piko-sync-client's settable fields.
 *
 * Same reasoning as protocol-test.cxx: no FLTK, no X, no device. Every
 * test works in a scratch directory under $TMPDIR (or /tmp), which it
 * removes on the way out, and points $HOME/$XDG_CONFIG_HOME there while
 * checking the default-path behavior -- so running this never touches
 * the real ~/.config/piko-sync/settings.cfg.
 *
 *   g++ -O2 -Wall -Wextra -o settings-test tests/settings-test.cxx && ./settings-test
 */
...
static void test_set_replaces_rather_than_appends()
{
    ...
    /* Not just "the last one wins on read" -- the FILE must contain one
     * line, or a long-lived config would grow a new line per save. */
...
static void test_unknown_keys_survive_a_save()
{
    ...
    Settings s;
    s.load_from(path);
    s.set("transfer.address", "10.0.0.9"); /* what this build knows about */
    check(s.save_to(path), "save_to() reports success");
...
static void test_get_int_rejects_junk()
{
    ...
    /* The checkbox case this exists for: junk must not read as "off". */
...
{
    /* A path whose parent is a FILE, not a directory: mkdir -p can't
     * make it, so save_to() must return false rather than abort. */
...
static void test_default_path_follows_home_and_xdg()
{
    ...
    /* The spec says a relative XDG_CONFIG_HOME must be ignored. */
...
int main()
{
    ...
    /* Saved and restored around the tests that move $HOME, so this
     * process can't be left pointing at the scratch dir if a check
    ...
    /* rm -rf the scratch tree. Shelling out beats hand-rolling an
     * nftw() walk for a test fixture. */
