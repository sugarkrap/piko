...
int jffs2_flash_read(struct jffs2_sb_info *c, loff_t ofs, size_t len, size_t *retlen, u_char *buf)
{
	...
	if ( (ret == -EBADMSG || ret == -EUCLEAN) && (*retlen == len) ) {
		/*
		 * RATE-LIMITED (2026-07-29, piko project): this fires once per
		 * mtd_read() call during the initial medium scan, which walks
		 * the entire partition -- on real (aged) hardware that can mean
		 * this printing for every single block, taking a very long time
		 * and making the framebuffer console, our only pre-userspace
		 * output on this device (no serial/USB, see AGENTS.md), useless
		 * for reading whatever comes after. The five lines below are
		 * unchanged: this warning was already non-fatal by design (ret
		 * is reset to 0, raw data is kept, node checksums sort out
		 * what's real) -- only the flood is silenced, not the handling.
		 */
		if (ret == -EBADMSG)
			pr_warn_ratelimited("mtd->read(0x%zx bytes from 0x%llx) returned ECC error\n",
				len, ofs);
		/*
