...
/*
 * Reverted 2026-08-03: the 2026-07-26 "recalibration" (offset -40 from these
 * mainline stock values) assumed this pack peaks around raw ADC 140-145, but
 * that was wrong -- live readings on this board (raw ADC ~198 on the BATT_VOLT
 * channel while unplugged) sit well above even the old table's 173 top entry,
 * so every real reading saturated at the top bucket and the battery meter
 * always showed 100%. Back to mainline's stock values, which assume a fresh
 * battery peaking around raw ADC 213 -- consistent with what this board
 * actually reports. See corgi_pm_patched.c for the matching status_high/low
 * revert.
 */
