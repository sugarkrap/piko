...
static const struct dma_slave_map pxa25x_slave_map[] = {
	 ...
	 *
	 * pxa2xx-i2s rx/tx (DREQ 2/3) is present in pxa27x_slave_map but was
	 * missing here -- the I2S DMA request-line block is unchanged between
	 * PXA25x and PXA27x (unlike SSP, which differs and has its own
	 * "PXA25x specific" entries below), so it belongs in this shared
	 * section. Without it, snd_dmaengine_pcm_open() on Corgi-class boards
	 * (PXA255 + WM8731 over I2S) gets a NULL channel from
	 * dma_request_slave_channel() and ASoC fails PCM open with -ENXIO,
	 * even though the sound card registers fine and userspace ALSA
	 * (aplay/libasound) is otherwise working correctly (found 2026-07-30
	 * chasing a "No such device or address" aplay error after the
	 * userspace ALSA static-build path had already been fully verified).
	 */
