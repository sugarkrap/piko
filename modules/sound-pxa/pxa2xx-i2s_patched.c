		...
		unsigned int fmt)
{
	...
	/*
	 * PATCHED: the clock-provider cases were wrong, which left this
	 * controller permanently in bit-clock-slave mode on Corgi.
	 *
	 * The SND_SOC_DAIFMT_{BP,BC}_{FP,FC} aliases are CODEC-centric --
	 * include/sound/soc-dai.h defines BP_FP as CBP_CFP ("codec clk
	 * PROVIDER"), not "CPU is provider" as the names read. The original
	 * pre-rename driver mapped CBS_CFS (codec bit slave + frame slave,
	 * i.e. the CPU drives both clocks) to master = 1, and CBM_CFS to
	 * master = 0. CBS_CFS is spelled CBC_CFC today, so the correct
	 * cases are CBC_CFC -> 1 and CBP_CFC -> 0; the BP_FP/BC_FP labels
	 * used here were CBP_CFP/CBC_CFP, i.e. inverted and mismatched.
	 *
	 * corgi.c asks for SND_SOC_DAIFMT_CBC_CFC (4 << 12), which matched
	 * NEITHER old case and fell through `default: break;`, so
	 * pxa_i2s.master kept its static initial value of 0. hw_params then
	 * never set SACR0_BCKD, so the PXA never drove BITCLK/LRCLK even
	 * though the codec was configured as a clock consumer. The result:
	 * the I2S link enables (SACR0_ENB=1, SACR1_DRPL=0) but no serial
	 * clock is ever generated, so the Tx FIFO never drains, SASR0_TFS
	 * never asserts, the I2S DMA request line never fires, and the
	 * already-armed DMA channel (DCSR RUN=1, DRCMR3 MAPVLD=1) sits idle
	 * forever -- ALSA reports state RUNNING with hw_ptr frozen at 0 and
	 * /proc/interrupts shows zero pxa-dma interrupts, while playback
	 * either hangs or eventually fails with -EIO. Confirmed on hardware
	 * 2026-07-30 by dumping SACR0/SASR0/DRCMR3/DCSR over /dev/mem
	 * (BCKD read back 0 with ENB=1 and TFS stuck low).
	 */
	switch (fmt & SND_SOC_DAIFMT_CLOCK_PROVIDER_MASK) {
	case SND_SOC_DAIFMT_CBC_CFC:
		/* codec consumes both clocks -> this controller drives them */
		pxa_i2s.master = 1;
		break;
	case SND_SOC_DAIFMT_CBP_CFC:
		/* codec provides the bit clock -> we consume it */
		pxa_i2s.master = 0;
		break;
	default:
		break;
