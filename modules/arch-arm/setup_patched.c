...
/*
 * piko_blink -- minimal "kernel is alive" heartbeat, added during the
 * 2026-07-29 mtd1 bootstrap-boot bring-up. Blinks BOTH LEDs @n times, then
 * pauses. Exactly one call site survives in the tree (see start_kernel() in
 * init/main.c); everything else that used to call this or the now-removed
 * piko_blink_number() sibling was bisection scaffolding and has been
 * stripped now that the boot hang is understood and fixed.
 */
#define PIKO_BLINK_DELAY_C	12000000UL
#define PIKO_GROUP_DELAY_C	48000000UL
#define PIKO_FAST_DELAY_C	3000000UL

/*
 * Global (not static) so callers elsewhere in the tree can share this one
 * copy. Deliberately NOT __init: it is called from __init code and keeping
 * it in .text avoids section-mismatch noise.
 *
 * n == 0 means "blink fast, forever".
 */
void piko_blink(unsigned int n)
{
	void __iomem *gpio = (void __iomem *)0x40E00000;
	void __iomem *scoop = (void __iomem *)0x10800000;
	volatile unsigned long d;
	unsigned int i;

	writel(readl(gpio + 0x0C) | (1 << 13), gpio + 0x0C);	/* GPDR0: out */
	writew(0x0140, scoop + 0x00);				/* SCOOP_MCR */
	writew(0x0000, scoop + 0x0C);				/* SCOOP_CPR */
	writew(1 << 1, scoop + 0x20);				/* SCOOP_GPCR */

	if (n == 0) {
		for (;;) {
			writel(1 << 13, gpio + 0x18);
			writew(1 << 1, scoop + 0x24);
			for (d = 0; d < PIKO_FAST_DELAY_C; d++)
				;
			writel(1 << 13, gpio + 0x24);
			writew(0, scoop + 0x24);
			for (d = 0; d < PIKO_FAST_DELAY_C; d++)
				;
		}
	...
	for (i = 0; i < n; i++) {
		writel(1 << 13, gpio + 0x18);			/* GPSR0 on */
		writew(1 << 1, scoop + 0x24);			/* GPWR on */
		for (d = 0; d < PIKO_BLINK_DELAY_C; d++)
			;
		writel(1 << 13, gpio + 0x24);			/* GPCR0 off */
		writew(0, scoop + 0x24);			/* GPWR off */
		for (d = 0; d < PIKO_BLINK_DELAY_C; d++)
			;
	}
	for (d = 0; d < PIKO_GROUP_DELAY_C; d++)
		;
}
