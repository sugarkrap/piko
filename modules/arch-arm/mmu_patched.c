...
static __init void prepare_page_table(void)
{
	...
	for (addr = 0; addr < MODULES_VADDR; addr += PMD_SIZE) {
		/*
		 * FIX (2026-07-29, piko project): preserve the extra
		 * identity-mapped GPIO and SCOOP sections added in
		 * __create_page_tables() (head.S). SCOOP in particular was
		 * silently unmapped before this: prepare_page_table() runs at
		 * the very start of paging_init() and would otherwise wipe
		 * both sections before iotable_init() ever gets a chance to
		 * remap them, breaking any post-MMU-pre-devicemaps_init()
		 * access to those registers (including the sole surviving
		 * piko_blink() heartbeat call in start_kernel()).
		 */
		if ((addr & PMD_MASK) == (0x40E00000 & PMD_MASK) ||
		    (addr & PMD_MASK) == (0x10800000 & PMD_MASK))
			continue;
		pmd_clear(pmd_off_k(addr));
	}
#endif
