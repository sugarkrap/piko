...
/*
 * PATCHED (2026-07-28): struct pxa25x_udc's 'mach' field
 * (struct pxa2xx_udc_mach_info *) replaced with 'pullup_gpio'
 * (struct gpio_desc *). pxa25x_udc.c was already updated to fetch the
 * D+ pullup control line via devm_gpiod_get_index_optional(..., "pullup",
 * ...) instead of the old platform_data-based mach->gpio_pullup +
 * gpio_is_valid()/mach->udc_command() path -- pxa2xx_udc_mach_info
 * itself is gone from mainline (see README.md's corgi_patched.c section:
 * "pxa_set_udc_info() / pxa2xx_udc_mach_info / udc.h are all gone").
 * This header just hadn't been updated to match yet, so the driver .c
 * referenced a struct member that didn't exist.
 */
